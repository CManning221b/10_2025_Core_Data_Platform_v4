...
class FileHistogramMethod(MethodImplementation):
    ...
    def execute(self, graph_manager, parameters):
        ...
        # Check if there's actually data to analyze
        has_data_to_analyze = False
        ...
        if property_type == 'extension':
            ...
            for node_id, node_data in graph_manager.node_data.items():
                if node_data.get('type') == 'extension':
                    has_data_to_analyze = True
                    break
        elif property_type == 'file_size':
            # Check for file_size nodes
            for node_id, node_data in graph_manager.node_data.items():
                if node_data.get('type') == 'file_size':
                    has_data_to_analyze = True
                    break

        if not has_data_to_analyze:
            print(f" No {property_type} data found - skipping histogram method execution")
            return changes

        # Only create method instance if there's data to analyze
